...
The smallest safe authorization is one non-publishing run on an already prepared disposable CI worker (or paired native amd64/arm64 workers) with Docker Buildx/QEMU/binfmt already available, Trivy and Syft installed, and isolated MariaDB 11.4/Redis 7.4 services. It needs permission only to check out the exact candidate commits, build and test locally, and retain redacted artifacts; it does not need registry publication, production credentials, NAS mutation, OCI mutation, DNS/OAuth/scheduler changes, or privileged installation on the NAS. If the existing GitHub workflow is used, the only additional authority needed is a single non-publishing workflow dispatch.

## Disposition

Task 6 package status: **DONE_WITH_CONCERNS**. Source/application/provider/client verification and native-amd64 browser integration are strongly evidenced, but G0 remains **HOLD** until every item above passes and the immutable release-identity manifest validates.
