...
**Amended 2026-08-27:** worker selection below is superseded by the [OCI A1 G0 worker harness design](../superpowers/specs/2026-08-26-oci-g0-worker-harness-design.md) (2026-08-26). The Synology DS718+ is no longer in use — its spike proved DSM's kernel cannot provide the required modern `binfmt_misc` contract — and the dedicated `racetime` A1 instance is the worker instead, building `linux/arm64` natively and `linux/amd64` under a transactional, cleanup-verified handler. The [Synology design](../superpowers/specs/2026-08-25-synology-g0-worker-harness-design.md) remains authoritative for qualification phases, artifact linkage, release-identity correction, evidence schema, and two-stage closeout. Harness implementation is in progress in `scripts/g0/` and `tests/g0/`. The paragraph below is retained unaltered as the 2026-08-24 record.
