...
The smallest safe authorization is access to one prepared, isolated, non-production Linux qualification worker with Docker Buildx, QEMU/binfmt, Trivy, Syft, and systemd support already installed, plus permission to check out the exact candidate commits, execute the checked-in qualification runbooks, and retain redacted artifacts. That authorized run must cover the same-commit dual-architecture build, smoke, Trivy, Syft SBOM, and provenance checks; isolated MariaDB 11.4/Redis 7.4 CI; secret-safe production Compose rendering; backup/restore container rehearsals; TTPBot `systemd-analyze security`; inherited Restream-history baseline review; release-identity collection; and final gate validation. The authorization does not include registry publication, production credentials, NAS mutation, privileged installation, OCI mutation, DNS/OAuth/scheduler changes, or any G1 action.
