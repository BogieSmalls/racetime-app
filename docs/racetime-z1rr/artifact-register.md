...
| PLT-010 | G1 | DNS/OAuth/secrets inventories (private) | Production secret and external-app records | Primary-operator access, redirect URI review, secret scan, and sealed recovery-package version/fingerprint/custody confirmation |
...
| OPS-009 | G3 | private secret/access inventory | Current operator, owners, clients, webhooks, policies, recovery copies | Primary operator verifies active access; recovery custodian acknowledges the current sealed package; stale access removed |
