...
| PLT-002 | G0 | `deploy/compose.production.yml` | Internal web/racebot/MariaDB/Redis/Caddy/maintenance stack with distinct qualification/production state | Both rendered phases select exactly their named DB/Redis/media/secret/Caddy volumes; fixed two-member proxy IPAM and separate data network; no DB/Redis host ports |
| PLT-003 | G0 | `deploy/Caddyfile`, `deploy/caddy/*.env.example` | Canonical HTTPS/WSS ingress, post-handshake default-deny, pinned ACME phases, and loopback-only admin | Adapted qualification/production configs each have one issuer with `ca == test_ca`, TLS-ALPN-only/no ZeroSSL; route-class denial and allowlisted HTTP/WSS tests pass; public `/admin` is unavailable |
| PLT-004 | G0 | `.env.production.example`, `deploy/env/ci.env`, `deploy/validate-config.py` | Secret-free environment contracts and validator | Placeholder/unknown/insecure values fail; fixture restricted/public phase, HSTS, production config, and fixed Caddy `/32` assertions pass |
...
| ID | Gate | Repository/path | Artifact | Acceptance evidence |
| --- | --- | --- | --- | --- |
| OPS-001 | G0 | `docs/runbooks/deploy.md`, `rollback.md` | Routine/emergency deploy and migration rollback | Primary-operator tabletop plus recovery-custodian possession/release-procedure confirmation |
| OPS-002 | G0 | `docs/runbooks/backup-restore.md`, `vm-loss.md` | Backup/restore/rebuild and account-access recovery procedures | Empty-host restore rehearsal meets RPO/RTO targets and identifies the verified OCI/GitHub/registry/DNS recovery route |
...
| OPS-005 | G0 | `deploy/monitoring/` | Health probes, metrics/log rules, redaction, Discord adapter, forecast-relative/high-forecast A1 utilization and independent storage-cost controls | Synthetic health, below-2,650 forecast-buffer/slope, at-or-above-2,650 forecast-record suppression, 2,900-hour utilization, Restream duty-cycle diagnostic, retained-volume, and Object Storage cases reach the test sink; secret canaries never appear in logs |
...
| OPS-007 | G2 | `docs/evidence/<date>-restore.json` | Isolated ARM64 and amd64 fresh-production full-restore evidence manifest at the recorded recovery target | Accounts/category/race/media/production-Caddy-state samples verify on both architectures; qualification state is ineligible; measured RPO and shape are recorded and the paid amd64 fallback restores within the four-hour RTO |
...
| OPS-009 | G3 | private secret/access inventory | Current operator, owners, clients, webhooks, policies, recovery copies and account-level recovery routes | Primary operator verifies active access and OCI/GitHub/registry/DNS recovery route; recovery custodian acknowledges the current sealed package; stale access removed |
