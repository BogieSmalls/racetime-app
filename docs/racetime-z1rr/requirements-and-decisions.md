...
The 1-OCPU/6-GB configuration is the locked launch and recovery target, not a starting point for an automatic resize. Because the accepted storage overage requires a paid tenancy, G1 must verify the applicable A1 entitlement. Oracle's current official sources diverge: the paid-tenancy price list states 3,000 OCPU-hours and 18,000 GB-hours monthly, while the Always Free resource page states 1,500/9,000 and also says paid accounts retain Always Free resources. The tenancy's recorded Limits, Quotas and Usage plus Cost Analysis are operational authority. A mismatch blocks apply only until operators reprice compute and Restream headroom and the Council records the revised decision.

Run NFR-PERF-001 early in G2. A failure blocks G2 and G3, requires profiling and optimization on the same 1-OCPU/6-GB configuration, and requires the complete gate to pass on retest. No CPU increase is authorized by this design; any future resize requires a new Council-approved architecture decision with revised RaceTime cost, Restream headroom, and disaster-recovery evidence. If the verified entitlement is 1,500/9,000, the locked RaceTime instance leaves 756 OCPU-hours, or 63 aggregate 12-OCPU Restream encoder-hours.
...
- **FR-OPS-007:** Rebuild from Git, immutable multi-platform images, configuration, operator-held secrets, and Object Storage on the dedicated `racetime` A1 VM or a temporary paid Linux amd64 shape. The paid fallback is fixed at 1 OCPU/6 GB and must pass the same recovery and performance gates; changing either production or fallback CPU count requires a new Council-approved architecture decision.
