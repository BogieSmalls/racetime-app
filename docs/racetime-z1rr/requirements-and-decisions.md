...
The 1-OCPU/6-GB target remains the launch default. Because the accepted storage overage requires a paid tenancy, G1 must verify the applicable A1 entitlement. Oracle's current official sources diverge: the paid-tenancy price list states 3,000 OCPU-hours and 18,000 GB-hours monthly, while the Always Free resource page states 1,500/9,000 and also says paid accounts retain Always Free resources. The tenancy's recorded Limits, Quotas and Usage plus Cost Analysis are operational authority. A mismatch blocks apply only until operators reprice compute and Restream headroom and the Council records the revised decision.

Run NFR-PERF-001 early in G2. A failure requires a planned in-place reboot resize to 2 OCPUs/6 GB and a full retest before final G2 qualification, not a launch waiver. If the verified entitlement is 1,500/9,000, a 1-OCPU RaceTime instance leaves 756 OCPU-hours, or 63 aggregate 12-OCPU Restream encoder-hours; a 2-OCPU RaceTime instance leaves only 12 OCPU-hours, or one aggregate encoder-hour. G2 cannot complete after that resize until the Council approves how Restream capacity will be preserved or rehomed.
...
- **FR-OPS-007:** Rebuild from Git, immutable multi-platform images, configuration, operator-held secrets, and Object Storage on the dedicated `racetime` A1 VM or a temporary paid Linux amd64 shape. The paid fallback uses the same qualified OCPU count as production and 6 GB RAM unless a separately approved Council exception changes the recovery/performance target.
...
- **NFR-PERF-001:** Early in G2, a 1-OCPU/6-GB ARM64 host must sustain at least twice the largest expected TTP room's browser/chat/reconnect load with 30% memory and 20% CPU burst headroom after steady state. Failure triggers a planned in-place resize to 2 OCPUs/6 GB and full retest before G2 completion; it cannot be waived for G3. Before G3, the recorded amd64 fallback must pass the same gate at the same qualified OCPU count and 6 GB RAM; temporary DR performance degradation is not an implicit exception. If the verified A1 entitlement leaves insufficient Restream encoder capacity after a required resize, G2 also requires a Council-approved Restream capacity or rehosting decision.
...
- **NFR-COST-001:** Create one Council-approved 50-GB Balanced boot volume for the dedicated `racetime` VM, with expected incremental storage/performance cost of approximately $2.13 per month at the current published US list price; do not create further retained volumes without Council approval. G1 evidence records expected A1 compute as exactly $0.00, forecast retained-storage as approximately $3.61, and the resulting combined forecast as an exact dollar value. Any nonzero A1 compute line triggers an immediate entitlement/reforecast warning; combined forecast or actual spend above baseline plus $1 warns and above baseline plus $3 escalates. Reconcile the first complete billing cycle against the recorded entitlement and forecast. Object Storage is outside the $3.61 retained-boot-volume projection: record the tenancy's applicable free entitlement, current tier usage, first full-backup size, and request count at G1; alarm at 75% and 90% of the verified byte/request entitlements, and require Council approval before forecast charges or retention expansion.
