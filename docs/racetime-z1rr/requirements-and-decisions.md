...
While the host is still restricted under G2, operators stop qualification schedulers, seal qualification backups under a restore-ineligible `qualification/` prefix, create fresh production volumes and secrets, perform a controlled Compose stop/repoint/start onto them, revoke qualification OAuth/bot/alert credentials, invalidate qualification sessions and tokens, bootstrap final production state, and rerun final smoke and dress-rehearsal checks. Only then may the Council grant G3. Rollback must never restore qualification data, backups, sessions, tokens, or credentials.

Pre-G3 access is a Caddy default-deny source-IP allowlist applied before every application, static, media, OAuth, and WebSocket route. A root-owned expiring record contains exact CIDRs for the primary and backup operators, approved scheduled testers, `coop-relay`, and required Restream hosts; it has no shared HTTP password. Two operators approve every entry and its expiry. Qualification uses Let's Encrypt's staging ACME endpoint as Caddy's only issuer, with no production/ZeroSSL/test-directory fallback and with dedicated test trust stores, until late G2. After fresh production state is active, operators switch once to a separate persistent production Caddy state volume and the Let's Encrypt production endpoint as the only issuer, using TLS-ALPN-01 with HTTP-01 disabled; the source-IP filter applies after the ACME TLS handshake, so no public application-path exemption exists. Production issuance makes the hostname visible in public Certificate Transparency logs, an accepted late-G2 disclosure after G1 activation. Unlisted public probes must receive a generic denial and cannot fetch assets, reach OAuth callbacks, or upgrade WebSockets.
...
- **FR-OPS-007:** Rebuild from Git, immutable multi-platform images, configuration, operator-held secrets, and Object Storage on the dedicated `racetime` A1 VM or a temporary paid Linux amd64 shape.
...
- **NFR-PERF-001:** Early in G2, a 1-OCPU/6-GB ARM64 host must sustain at least twice the largest expected TTP room's browser/chat/reconnect load with 30% memory and 20% CPU burst headroom after steady state. Failure triggers a planned in-place resize to 2 OCPUs/6 GB and full retest before G2 completion; it cannot be waived for G3.
