...
**G0 stop line:** Do not edit `/etc/ttpbot.env`, stop/start the production service, rotate OAuth credentials, create a production room, or send a production Discord webhook. BOT-006 executes late in G2 against fresh restricted production only after production certificate issuance, using ordinary certificate validation and isolated qualification schedule/webhook credentials.
