...
The expected file carries live OCIDs/IDs plus the reviewed binary-plan digest and remains
ignored. Parse resource changes, resource drift, output changes, planned values, unknown
values, and configuration references. Hash the binary plan internally and compare it to
the expected digest. Use `git status --porcelain` to require exact HEAD, no tracked
changes, and no non-ignored untracked files. Print only a
...
route/DHCP IDs, binary-plan SHA-256, and source commit, then run:
...
`--preserve-boot-volume false`, `--force`, and `--wait-for-state SUCCEEDED`. Do not use
display-name selection. Then poll the exact instance OCID until lifecycle state is
`TERMINATED`; require the exact boot-volume get/list result to show `TERMINATED` or absent
and prove it is not among retained volumes. A 412/ETag mismatch is a clean stop.
